  ...
  return (
    <Card className="rounded-2xl border-border/60 p-5 shadow-soft">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold text-sm">Weekly Utilization Heatmap</h3>
              ...
              {hours.map((hour) => {
                ...
                return (
                  <div
                    key={hour}
                    className={cn(
                      "aspect-square rounded-sm flex items-center justify-center text-xs font-medium transition-colors",
                      getColor(hoursBooked),
                      hoursBooked > 0 ? "text-primary-foreground" : "text-muted-foreground",
                    )}
                    title={`${day} ${hour}:00 - ${fmtHours(hoursBooked)}h`}
                  >
                    {hoursBooked > 0 ? fmtHours(hoursBooked) : ""}
                  </div>
                );
              })}
            </div>
