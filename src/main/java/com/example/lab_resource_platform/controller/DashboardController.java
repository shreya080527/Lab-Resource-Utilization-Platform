...
public class DashboardController {
    ...
    public ResponseEntity<Map<String, Object>> stats() {
        ...
        // Get current user for department-scoped stats
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        User currentUser = null;
        if (auth != null && auth.getPrincipal() instanceof UserPrincipal) {
            UserPrincipal principal = (UserPrincipal) auth.getPrincipal();
            currentUser = userRepo.findByEmail(principal.getUsername()).orElse(null);
        }
        
        // Check if user is SYSTEM_ADMIN or INSTITUTION_ADMIN (see all)
        boolean canSeeAll = currentUser != null && 
            (currentUser.getRole() == Role.SYSTEM_ADMIN || currentUser.getRole() == Role.INSTITUTION_ADMIN);
        
        // Equipment counts (always system-wide for simplicity)
        ...
        // Booking counts - department-scoped for LAB_MANAGER and DEPARTMENT_HEAD
        if (canSeeAll) {
